...
%%%% Document Structure %%%%
	...
	\NewDocumentCommand{\header}{m m m}{% Makes header that links to the key="top", use in conjunction with \thetitle
		\fancyhead[L]{#1} 
...
%%%% Math %%%%
	...
	%% Symbols and Shorthand
		% to make sets
		\LoopCommands{\lettersAll}[#1set] {\newmathcommand#2{\{\MakeUppercase{#1}\}}} 
		\LoopCommands{\lettersAll}[#1set] {\newtextcommand#2{\(\{\MakeUppercase{#1}\}\)}}
		...
		\LoopCommands{{l}{a}}[d#1]{\NewDocumentCommand#2{s}{\IfBooleanTF{##1}{\cdot}{}\d\mathbf{#1}}}
		\LoopCommands{xyztV}[d#1]{\NewDocumentCommand#2{s}{\,\d#1}}
		%\NewDocumentCommand\dl{s}{\IfBooleanTF{#1}{\cdot}{} \d\mathbf{l}}% Curve integral •dl, if no star, if no star then just dl
		%\NewDocumentCommand\da{s}{\IfBooleanTF{#1}{\cdot}{} \d\mathbf{a}}% Surface integral •da, if no star then just da 
